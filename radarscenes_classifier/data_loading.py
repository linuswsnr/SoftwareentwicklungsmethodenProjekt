...
def save_sequence_pickle(seq_path: str, output_dir: str) -> None:
    ...
    seq_number = (
        seq_name.split("_")[1] if "_" in seq_name else seq_name
    )
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(
        output_dir,
        f"DataSeq_{seq_number}.pkl"
    )
...
def generate_all_sequence_pickles(base_path: str, output_dir: str) -> None:
    """Erstellt Pickle-Dateien für alle Sequenzen
    unter base_path im output_dir."""
    seq_dirs = sorted(
    ...
    if not seq_dirs:
        raise FileNotFoundError(
            f"Keine Sequenzen in {base_path} gefunden."
        )
